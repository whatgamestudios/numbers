...
import {FourteenNumbersClaim} from "./FourteenNumbersClaim.sol";
 ...
 */
contract FourteenNumbersClaimV2 is FourteenNumbersClaim {
    error IncorrectPreviousVersion(uint256 _ver);
    ...
    function upgradeStorage(bytes memory /* _data */) external override virtual {
        if (version != _VERSION0) {
            revert IncorrectPreviousVersion(version);
        }
        version = _VERSION1;
    }
     ...
     */
    function prepareForClaim(uint256 _salt) external override whenNotPaused() {
        // Do nothing
    }
     ...
     * @notice Claim an NFT. 
     */
    function claim(uint256 /* ignored */) external override whenNotPaused() {
